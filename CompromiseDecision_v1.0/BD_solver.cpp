...
bd_solution bd_master(standardTwoStageParameters& model_parameters,
    ...
    //model.write("/Users/shuotaodiao/Documents/Research/numerical_experiments/CD/lands/lands_model_master_large.lp");
...
bd_output bd_solver(const std::string& folder_path,
    ...
    std::string resultsOutput_path = folder_path + "/computationalResults(rep_BDv1.0).txt";
    ...
    double z_est;
    bd_subproblem_cuts subproblem_cons;
    ...
    while (flag_terminate == false) {
        std::cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n";
        writeFile << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n";
        std::cout << "(BD) Iteration:  " << it_num << std::endl;
        writeFile << "(BD) Iteration:  " << it_num << std::endl;
        // solve the relaxed master problem
        // update estimated solution (step 1 of the Benders decomposition)
        bd_solution est_sol = bd_master(model_parameters, subproblem_cons);
...
compromise_bd_output bd_compromise_solver(const std::string& folder_path,
    ...
    // Initialization
    std::string model_path = folder_path + "/model.txt";
    std::string resultsOutput_path = folder_path + "/computationalResults(CD_BDv1.0).txt";
    std::string sto_path = folder_path + "/sto.txt";
    // create sto object
    stoMap stochastic_map;
    // read sto file
    ...
    std::vector<bd_output> aggregate_outputs;
    for (int idx_rep = 0; idx_rep < num_replications; ++idx_rep) {
        bd_output replication_outputs = bd_solver(folder_path, generator, sample_size_per_replication, error);
    ...
    compromise_bd_output compromise_res = bd_compromise_master(model_parameters, aggregate_outputs, regularizer_coefficient);
